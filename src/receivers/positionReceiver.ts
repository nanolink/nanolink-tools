...
class PositionReceiver {
   ...
   * @param {string[]} [trackers] - Filter on specific trackers
   * @returns {*}
   */
  async run(unwind?: boolean, subscribe?: boolean, trackers?: string[]) {
    let iter = await this.connection.subscribe(
      StateSubscriptions.positions,
      { subscribe: subscribe ?? true,  trackers },
      unwind
    );
